				...
				success : function(dataObj) {
					...
					if(dataObj.length==0){//没有数据
						...
						$(".data-count tr").dblclick(function(){
							$.ajax({
								url:"{0}/send/upload".format($.getRequestUrl()),
								type:"post",
								data:{
									"ewbNo":dataObj.ewbNo,
...
function getNextSite(){
	$.ajax({
		url: "{0}/send/getNextSite".format($.getRequestUrl()),
		type: "post",
		data:{
			"ewbNo":$("#ewbNo").val()
		},
